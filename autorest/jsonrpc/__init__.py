...
import logging
import \
    logging.config  # need to include this extra import so mypy doesn't throw
# logging module has no config
...
class Channel(Enum):
    # Information is considered the mildest of responses; not necesarily
    # actionable.
    Information = "information"

    # Warnings are considered important for best practices, but not
    ...
    # Debug messages are designed for the developer to communicate internal
    # autorest implementation details.
    Debug = "debug"
...
class AutorestAPI(ABC):
    ...
    def get_boolean_value(self, key: str, default: bool = None) -> Optional[
        bool]:
        """Check if value is present on the line, and interpret it as bool if
        it was.

        If value is not not on the line, return the "default".
        If the value is present, will also accept "true" or 1 as True.

        For autorest, empty dict means "it was on the line", so we want it to
        true.
